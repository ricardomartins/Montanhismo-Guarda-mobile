 ...
 */
sealed class WatchCase<out R>(protected val dispatcher: CoroutineDispatcher) {
    abstract class Supplier<out R>(dispatcher: CoroutineDispatcher) : WatchCase<R>(dispatcher) {
    ...
    abstract class Function<in P, out R>(dispatcher: CoroutineDispatcher) : WatchCase<R>(dispatcher) {
        protected abstract fun execute(param: P): Flow<R>

        operator fun invoke(param: P): Flow<R> = execute(param).flowOn(dispatcher)
    }

    abstract class BiFunction<in P1, in P2, out R>(dispatcher: CoroutineDispatcher) : WatchCase<R>(dispatcher) {
